...
save_root = 'exp/fedavg_gamma_lama'
...
## import config
cfg = Config()
config = cfg.generate_config(num_clients=4,fed_mode= 'hete',client_datasets=['vox1','vox1','vox1','vox1'],round=1)
...
args.exp_name='{}_gamma{}_{}_embed{}'.format(args.exp_name,args.gamma,args.reweight_method,args.embed_dim)
...
def train(model, client_index, epoch, loader):
    model.train()
...
def main(args):
    ...
    for i in range(args.num_clients):
        ...
        external_evaluators.append(Evaluator(args.external_test_lists[i], args.external_test_path[i]))
    ...
    while epoch <= args.max_epoch:
        ...
        for i in range(args.num_clients):
            ...
            # Training
            loss, lr, acc = train(client_models[i], i+1, epoch, client_train_loaders[i])
       ...
       # Update server with client model
        if epoch % args.round == 0:
            rw_EERs = []
            for i in range(args.num_clients):
                rw_EER, _, _ = external_evaluators[i].eval(client_models[i])
                rw_EERs.append(rw_EER)
                logger.log(
                    "{}  rw_EER:{:.4f}".format(datetime.datetime.now(), rw_EER),
                    time.strftime("%Y-%m-%d %H:%M:%S")
                )
            lambda_weights = compute_client_rw_weights(rw_EERs,args.reweight_method,scale=1.0,temprature=0.5)
            server_model.update_parameters_gamma(client_models, lambda_weights, args.gamma, mode=args.fed_mode)
